...
const updateUserProfileData = async() => {
  ...
  return userData &&  (<>
    <div className='max-w-lg flex flex-col gap-2 text-sm' >
      {
        ...
        ? <label htmlFor='image'>
          <div className='inline-block relative cursor-pointer'>
            <img className='w-35 rounded opacity-75' src={image ? URL.createObjectURL(image):userData.image} alt="" />
    ...
    <div>
      ...
      <div className='grid grid-cols-[1fr_3fr] gap-y-2.5mt-3 text-neutral-700'>
  ...
  <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
    <p className='font-medium'>Gender:</p>
      ...
      : <p className='text-gray-400'>{userData.gender}</p>
        ...
        {
      isEdit 
      ? <input className='max-w-28 bg-gray-100' type="date"  onChange={(e)=>setUserData(prev => ({...prev,dob : e.target.value}))} value={userData?.dob ? new Date(userData.dob).toISOString().split("T")[0] : ""}/>
